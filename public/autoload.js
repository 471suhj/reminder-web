...
export function fncRefresh(lblLoadMore, list, sortMode, fncInsertFile, fncPrintCnt, dirid){
    fncClearList(lblLoadMore);
    fncLoadMore(lblLoadMore, list, sortMode, fncInsertFile, fncPrintCnt, dirid);
}
...
export async function fncLoadMore(lblLoadMore, list, sortMode, fncInsertFile, fncPrintCnt, dirid){
    ...
    await doFetch(`/files/loadmore?dirid=${dirid}&sort=${sortMode.criteria}&sortincr=${sortMode.incr}&startafter=` + idCurLast, 'GET', '', '', '추가 로드에 실패했습니다.', async function(result){
...
export function fncAutoloadSetup(lblLoadMore, list, sortMode, fncInsertFile, fncPrintCnt, dirid){
    ...
    document.getElementById('refresh').addEventListener('click', () => {fncRefresh(lblLoadMore, list, sortMode, fncInsertFile, fncPrintCnt, dirid);});
