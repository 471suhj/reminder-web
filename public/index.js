    ...
    if (tryCnt < intEncrMaxCnt){
        if (objPubKey === null){
            await doFetch("/auth/encr", "GET", "", "", "정상적인 서버 통신에 실패했습니다.", async function(result){
                objPubKey = JSON.parse(await result.json().value);
            });
        }
        if (objPubKey === null){
            return;
        ...
        const ckPubKey = await window.crypto.subtle.importKey("spki", objPubKey, {name: "RSA-OAEP", hash: "SHA-256"}, false, ["encrypt"]);
        ...
        jsnReq.key = JSON.stringify(objPubKey);
    } else {
        jsnReq.nokey = "true";
    }
